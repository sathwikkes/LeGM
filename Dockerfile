...
# Player data lives here (SQLite by default); mount a volume or point LEGM_DATABASE_URL at Postgres.
RUN mkdir -p /app/data/raw && useradd --create-home --uid 10001 legm && chown -R legm:legm /app
USER legm
VOLUME ["/app/data"]

ENV PATH="/app/.venv/bin:$PATH" LEGM_CORS_ORIGINS="http://localhost:3000"
EXPOSE 8000
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s \
    CMD python -c "import urllib.request,sys; sys.exit(0 if urllib.request.urlopen('http://127.0.0.1:8000/api/health', timeout=3).status == 200 else 1)"

CMD ["uvicorn", "legm.api.app:app", "--host", "0.0.0.0", "--port", "8000", "--proxy-headers", "--forwarded-allow-ips=*"]
